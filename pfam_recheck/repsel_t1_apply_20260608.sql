...
\copy _cr FROM 'repsel_t1_create_final.csv' WITH CSV HEADER
...
BEGIN FOR r IN SELECT * FROM _cr LOOP
  v:=ecod_rep.create_domain_change_request(r.ecod_domain_id,r.ecod_source_id,r.f_id,r.seqid_range,
     NULL,false,true,r.dtype,'v294 rep-selection Tier-1','single-member repless F-group: sole member promoted as provisional rep, Pfam-concordant with pfam_acc','v294_repsel_t1',r.ecod_uid);
  PERFORM ecod_rep.approve_hierarchy_change_request(v,'v294_repsel_t1','repsel t1');
