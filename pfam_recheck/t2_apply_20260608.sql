...
BEGIN;
-- ---- 213 reassign (provisional reps already flagged; f_id move carries the flag) ----
CREATE TEMP TABLE _in(ecod_uid int, new_f_id text);
\copy _in FROM 't2_reassign_clean.csv' WITH CSV HEADER
...
BEGIN FOR r IN SELECT * FROM _w LOOP
  ...
  v:=ecod_rep.create_domain_update_request(r.uid,'reassign_f_group',r.new_f_id,
     'v294 reconciliation tranche-2: Pfam-validated provisional-rep f_group propagation','v294_t2');
  PERFORM ecod_rep.approve_hierarchy_change_request(v,'v294_t2','tranche2 reassign');
  PERFORM ecod_rep.implement_domain_update(v); n:=n+1;
END LOOP; RAISE NOTICE 'reassigned: %', n; END $$;
-- ---- 39 create (new provisional reps for repless targets) ----
CREATE TEMP TABLE _cr(ecod_uid int, f_id text, ecod_domain_id text, ecod_source_id text, seqid_range text, dtype text);
\copy _cr FROM 't2_create.csv' WITH CSV HEADER
DO $$ DECLARE r RECORD; v INT; n INT:=0;
BEGIN FOR r IN SELECT * FROM _cr LOOP
  v:=ecod_rep.create_domain_change_request(r.ecod_domain_id,r.ecod_source_id,r.f_id,r.seqid_range,
     NULL,false,true,r.dtype,'v294 tranche-2 reseed','Pfam-validated provisional rep for repless F-group','v294_t2',r.ecod_uid);
  PERFORM ecod_rep.approve_hierarchy_change_request(v,'v294_t2','tranche2 create');
