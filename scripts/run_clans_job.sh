...
# ECOD F-group Consistency Analysis - CLANS Job
...
# This prevents race conditions when multiple jobs run BLAST simultaneously
...
if [ -z "$SLURM_ARRAY_TASK_ID" ]; then
    ...
    echo "Usage: sbatch --array=1-N run_clans_job.sh"
    exit 1
fi

JOB_ID=$SLURM_ARRAY_TASK_ID

# Extract job info using Python (jq alternative)
JOB_INFO=$(python3 << EOF
import json
with open("${MANIFEST}") as f:
    manifest = json.load(f)

# Find job by job_id (1-indexed)
job = None
...
# Run CLANS from the unique temp directory to avoid blast_output race conditions
echo ""
echo "Running CLANS..."
...
# Change to job-specific temp directory so CLANS creates blast_output there
cd "$JOB_TMPDIR"
