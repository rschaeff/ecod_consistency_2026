#!/usr/bin/env python3
"""
Batch deprecation script for A1 singleton F-groups with simple_topology provisional reps.

This script handles F-groups where:
- Only 1 representative domain exists in ecod_rep.domain
- Only 1 domain is assigned in ecod_commons.f_group_assignments
- The representative is classified as simple_topology

Actions performed:
1. Delete the simple_topology domain from ecod_rep.domain (with audit trail)
2. Deprecate the F-group in ecod_rep.cluster
3. Reassign the domain in ecod_commons.f_group_assignments to the .0 pseudo F-group

Usage:
    python batch_deprecate_a1.py --dry-run          # Preview changes without executing
    python batch_deprecate_a1.py --execute          # Execute deprecations
    python batch_deprecate_a1.py --execute --batch-size 50  # Execute in batches of 50
"""
...
This F-group is a singleton (1 rep, 1 assignment) created from a structural fragment.
Actions taken:
- Domain deleted from ecod_rep.domain
- F-group deprecated in ecod_rep.cluster
- Domain reassigned to {pseudo_fgroup} in ecod_commons

Reference: DEFICIENCY_REPORT_605.1.md
Batch: simple_topology_deaccession_{date}"""
...
def load_a1_fgroups(classification_file: Path) -> list:
    """Load A1 singleton F-groups from classification results."""
    a1_fgroups = []
    with open(classification_file, 'r') as f:
        reader = csv.DictReader(f, delimiter='\t')
        for row in reader:
            if row['category'] == 'A1_SINGLETON_TOTAL':
                a1_fgroups.append({
                    'f_id': row['f_id'],
                    'h_group': row['h_group'],
                    'prov_rep_domain': row['prov_rep_domain'],
    ...
    with conn.cursor() as cur:
        ...
        cur.execute("""
            INSERT INTO ecod_rep.domain_modification_log (
                ...
                'Change request #' || %s || ' - Simple topology domain removed from ecod_rep',
                %s, NOW()
            )
        """, (
            domain['uid'],
        ...
        cur.execute("""
            ...
            SET f_group_id = %s,
                ...
                notes = COALESCE(notes, '') ||
                        E'\nReassigned from ' || %s || ' (deprecated simple_topology F-group) on ' ||
                        NOW()::text
            WHERE f_group_id = %s
...
def deprecate_fgroup(conn, fgroup: dict, dry_run: bool = True) -> dict:
    ...
    3. Reassign domains in ecod_commons to .0 pseudo F-group
    ...
    try:
        ...
        # Step 5: Reassign domains in ecod_commons to .0 pseudo F-group
...
def run_batch_deprecation(
    ...
    """
    Run batch deprecation of A1 singleton F-groups.
    """
    # Load F-groups to deprecate
    fgroups = load_a1_fgroups(classification_file)
    print(f"Loaded {len(fgroups)} A1 singleton F-groups for deprecation")
    ...
    if output_file:
        with open(output_file, 'w') as f:
            fieldnames = ['f_id', 'domain', 'pseudo_fgroup', 'status',
                          'request_id', 'domains_reassigned', 'error']
...
def main():
    parser = argparse.ArgumentParser(
        description='Batch deprecate A1 singleton F-groups with simple_topology provisional reps'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--classification-file', type=Path,
        default=Path('/home/rschaeff/work/ecod_consistency_2026/prov_rep_daccession/classification_results.tsv'),
        help='Path to classification results TSV'
    )
    parser.add_argument(
        '--output-file', type=Path,
        default=Path('/home/rschaeff/work/ecod_consistency_2026/prov_rep_daccession/deprecation_results.tsv'),
